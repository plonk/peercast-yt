...
void Template::writeGlobalVariable(Stream &s, const String &varName, int loop)
{
    ...
    // ïœêîÇ™å©ïtÇ©ÇÁÇ»Ç©Ç¡ÇΩèÍçáÇÕïœêîñºÇèëÇ´èoÇ∑
End:
    if (!r)
...
string Template::evalStringLiteral(const string& input)
{
    ...
    while (!s.empty() && s[0] != '\"')
    {
        if (s[0] == '\\')
        {
            // ÉoÉbÉNÉXÉâÉbÉVÉÖÇ™ç≈å„ÇÃï∂éöÇ≈ÇÕÇ»Ç¢Ç±Ç∆ÇÕÇÌÇ©Ç¡ÇƒÇ¢ÇÈ
            // ÇÃÇ≈ññí[É`ÉFÉbÉNÇÕÇµÇ»Ç¢ÅB
            res += s[0];
            res += s[1];
            s.erase(0,2);
        }else
...
json::array_t Template::evaluateCollectionVariable(String& varName)
{
    if (varName == "channelsFound")
    {
        ...
        // ÉWÉÉÉìÉãê⁄ì™é´Ç≈énÇ‹ÇÁÇ»Ç¢É`ÉÉÉìÉlÉãÇÕåfç⁄ÇµÇ»Ç¢ÅB
    ...
    {
        // Ç±ÇÃÉTÅ[ÉoÅ[Ç©ÇÁîzêMíÜÇÃÉ`ÉÉÉìÉlÉãÇÉäÉXÉiÅ[êîç~èáÇ≈É\Å[ÉgÅB
        JrpcApi api;
...
// in ÇÃåªç›ÇÃà íuÇ©ÇÁ 1 ÉuÉçÉbÉNï™ÇÃÉeÉìÉvÉåÅ[ÉgÇèàóùÇµÅAoutp Ç™
// NULL Ç≈Ç»ÇØÇÍÇŒ *outp Ç…èoóÕÇ∑ÇÈÅB{@loop} ì‡ÇèàóùÇµÇƒÇ¢ÇÈèÍçáÇÕÅA0
// Ç©ÇÁénÇ‹ÇÈÉãÅ[ÉvÉJÉEÉìÉ^Å[ÇÃílÇ™ loop Ç…ê›íËÇ≥ÇÍÇÈÅBEOF Ç†ÇÈÇ¢ÇÕ
// {@end} Ç…ìñÇΩÇ¡ÇΩèÍçáÇÕ false Çï‘ÇµÅA{@else} Ç…ìñÇΩÇ¡ÇΩèÍçáÇÕ true
// Çï‘Ç∑ÅB
bool Template::readTemplate(Stream &in, Stream *outp, int loop)
{
    ...
    while (!in.eof())
    {
        ...
        if (c == '{')
        {
            ...
            {
                // ÉeÉìÉvÉåÅ[ÉgÇ…ä÷åWÇÃÇ»Ç¢îgäáå ÇÕÇªÇÃÇ‹Ç‹ï\é¶Ç∑ÇÈ
                if (p)
                {
                    p->writeChar('{');
                    p->writeChar(c);
...
bool HTTPRequestScope::writeVariable(Stream& s, const String& varName, int loop)
{
    if (varName == "request.host")
    {
        ...
        return true;
    }else if (varName == "request.path") // HTTPRequest „Å´ÂßîË≠≤„Åô„Åπ„Åç„Åã
    {
